...
type Barang struct {
	...
	SuratIzin    string  `json:"surat_izin"`
...
func estimasiBiaya(b Barang) float64 {
	biayaPajak := float64(b.Harga) * b.PajakPersen / 100
	biayaTransport := float64(b.JarakKM) * 1500 // tarif/km
...
func heapSortPajak(barangs []Barang) {
	sort.Slice(barangs, func(i, j int) bool {
		return barangs[i].PajakPersen < barangs[j].PajakPersen
	})
